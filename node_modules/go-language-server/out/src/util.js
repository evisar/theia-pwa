...
const vscode_extension_telemetry_1 = require("../src-vscode-mock/vscode-extension-telemetry");
const fs = require("fs");
const os = require("os");
...
function getToolsEnvVars() {
    ...
    if (goroot && (envVars['PATH'] || '').split(path.delimiter).indexOf(goroot) === -1) {
...
function handleDiagnosticErrors(document, errors, diagnosticSeverity) {
    if (diagnosticSeverity === undefined || diagnosticSeverity === 1 /* Error */) {
        goMain_1.errorDiagnosticCollection.clear();
    }
    if (diagnosticSeverity === undefined || diagnosticSeverity === 2 /* Warning */) {
    ...
    diagnosticMap.forEach((diagMap, file) => {
        ...
        if (diagnosticSeverity === undefined || diagnosticSeverity === 1 /* Error */) {
            const newErrors = diagMap[1 /* Error */];
        ...
        if (diagnosticSeverity === undefined || diagnosticSeverity === 2 /* Warning */) {
            ...
            let newWarnings = diagMap[2 /* Warning */];
